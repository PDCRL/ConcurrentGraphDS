...
public:
  ...
  wait_free_graph(int no_of_threads)
  {
    ...
    {
      state[i] = init_op(0, success, NULL, NULL, NULL, NULL);
    }
  }
  ...
  void help(int tid, long int phase);
  ...
  void print_graph();
  bool is_search_pending(int id, int phase);
  ...
  void search_vertex(Node_List **prev, Node_List **curr, int key, int id, int phase);
  void search_edge(Node **vnode, Node_List **prev, Node_List **curr, int key, int id, int phase);
  long int next_phase();
...
void wait_free_graph ::locate_cplus(Node_List *startV, Node_List **n1, Node_List **n2, int key)
{
  ...
  predv = startV;
  currv = (Node_List *)get_unmarked_ref((long)predv->v_next.load()); //startV->v_next.load();
...
void wait_free_graph ::locate_vplusplus(Node_List *startV, Node_List **n1, Node_List **n2, int key)
{
  ...
  predv = startV;
  currv = (Node_List *)get_unmarked_ref((long)predv->v_next.load()); //predv->v_next.load();
...
bool wait_free_graph ::contain_node(int id, int key, int dest, bool is_vertex)
{
  ...
  {
    Node_List *curr = head;
    while (curr->v_next.load() && curr->data < key)
    {
      curr = (Node_List *)get_unmarked_ref((long)curr->v_next.load());
    }
  ...
  else
  {
    ...
    Node_List *u;
    Node_List *v;
    bool flag = contain_cplus(&u, &v, key, dest);
    if (flag == false)
    {
      return false;
    }
    curre = u->e_next.load();
    while (curre->e_next.load() && curre->data < dest)
    {
      curre = (Node_List *)get_unmarked_ref((long)curre->e_next.load());
    }
    if ((curre->e_next.load()) && curre->data == dest && !is_marked_ref((long)curre->e_next.load()) && !is_marked_ref((long)u->v_next.load()) && !is_marked_ref((long)v->v_next.load()))
    {
      return true;
    }
    else
...
void wait_free_graph ::help(int tid, long int phase)
{
  for (int i = 0; i < size; i++)
  {
    OpDesc *desc = state[i].load(std::memory_order_seq_cst);
    if (desc->phase <= phase)
    {
      if (desc->type == insert_vertex)
      {
        help_insert_vertex(i, desc->phase);
      }
      else if (desc->type == insert_edge)
      {
        help_insert_edge(i, desc->phase);
      }
      else if (desc->type == search_delete_vertex)
      {
        help_delete_vertex(i, desc->phase);
      }
      else if (desc->type == search_delete_edge)
      {
        help_delete_edge(i, desc->phase);
      }
    }
  }
}
...
void wait_free_graph ::print_graph()
{
  Node_List *temp = head;
  Node_List *etemp;
  while (temp != NULL)
...
void *pthread_call(void *t)
{
  ...
  long long int numOfOperations_addEdge = numOfOperations * 0.125;       // 25% for add edge
  long long int numOfOperations_addVertex = numOfOperations * 0.125;     // 25% for add vertex
  long long int numOfOperations_removeVertex = numOfOperations * 0.125;  // 10% for remove vertex
  long long int numOfOperations_removeEdge = numOfOperations * 0.125;    // 10% for remove edge
  long long int numOfOperations_containsVertex = numOfOperations * 0.25; // 15% for contains vertex
  long long int numOfOperations_containsEdge = numOfOperations * 0.25;   // 15% for contains edge
  ...
  while (total > 0)
  {
    ...
    if (other == 0)
    {
      ...
      l1:
        ...
        if (u == v || u == 0 || v == 0) //simple graph without self loops
          goto l1;
        res = G1->insert_node((int)Tid, u, v, false);
    ...
    else if (other == 1)
    {
      if (numOfOperations_addVertex > 0)
      {
        ...
        vertexID++;
        res = G1->insert_node((int)Tid, v, -1, true);
    ...
    else if (other == 2)
    {
      ...
      l2:
        ...
        if (v == 0)
          goto l2;
        res = G1->delete_node((int)Tid, v, -1, true);
    ...
    else if (other == 3)
    {
      ...
      l3:
        ...
        if (u == v || u == 0 || v == 0)
          goto l3;
        res = G1->delete_node((int)Tid, u, v, false);
    ...
    else if (other == 4)
    {
      ...
      l4:
        ...
        if (u == 0)
          goto l4;
        res = G1->contain_node((int)Tid, u, -1, true);
