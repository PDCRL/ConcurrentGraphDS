 ...
 *   
 * Description:
 *   coarse-lock implementation of a graph
 * Copyright (c) 2018.
...
void locateVPlus(vlist_t *startV, vlist_t ** n1, vlist_t ** n2, int key){
 ...
 bool AddV(int key){
  ...
  //pthread_mutex_lock(&lock);
        ...
        //cout<<" Add pred, curr, succ:"<<predv<<" "<<currv<<" "<<endl;
        if(currv->val == key){
...
void PrintGraph(){
	...
	while(temp1 != NULL){
		...
		cout << temp1->val<<" "<<temp1->ecount<<" "<< temp1->visitedArray[0]<<","<<temp1->visitedArray[1]<<","<<temp1->visitedArray[2] << "->"; // print vertex
		temp2 = temp1->enext;
		while(temp2 != NULL)
		{
			cout << "["<<temp2->val; // print edge node
			if(temp2->val != INT_MAX && temp2->val != INT_MIN) 
			  cout<<","<<(temp2->pointv)->val<<"]"; // print it's vertex node
			temp2 = temp2->enext;
		}
		cout << endl;
		temp1 = temp1->vnext;
