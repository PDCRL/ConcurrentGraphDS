...
int contains_edge(long long int u, long long int v)
{
	...
	if(pred->key != v || pred->marked.load() == true)		//edge already present
		return false;
...
void* pthread_call(void *t)
{
	...
	int other, res;
 	...
 	int numOfOperations_addEdge = numOfOperations * 0.2; 		// 20% for add edge
  	int numOfOperations_addVertex = numOfOperations * 0.2; 	// 15% for add vertex
  	int numOfOperations_removeVertex = numOfOperations * 0.2; 	// 15% for remove vertex
  	int numOfOperations_removeEdge = numOfOperations * 0.1; 	// 15% for remove vertex
  	int numOfOperations_containsVertex = numOfOperations * 0.2; 	// 15% for remove vertex
